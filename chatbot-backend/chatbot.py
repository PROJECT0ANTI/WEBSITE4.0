...
class Chatbot:
    ...
    weights = {"ai": 2, "web": 2, "devops": 2,'founded':2,'founder':2,'owner':2,'Anti':10,'located':4,'situated':2}    
    ...
    def get_response(self, input_quest):
        self.input_quest = input_quest
        ...
        for t in token :
            if t in Chatbot.weights:
                pass
            else:
                Chatbot.weights[t]=0
        filtered_with_weight=[token if token in Chatbot.weights else token for token in filtered_tokens]
#         print("Filtered with weight ",filtered_with_weight)
        filtered_tokens = sorted(filtered_with_weight, key=lambda token: Chatbot.weights[token], reverse=True)
#         general_question_lowerCase = list(map(lambda x: x.lower(), self.general_question))
        print("Filtered tokens and sorted ",filtered_tokens)
        ...
        if self.best_match:
            ...
            for tokens in filtered_tokens:
                ...
                if len(matches) == 1:
                    return f"{self.response[matches[0]]}"
                    ...
                    best=self.all_matches(filtered_tokens,matches)
                    print("best result ",best)
                    if best:
                        return "\n".join([f"{self.response[match]}" for match in best])

                    else:
                        return "\n".join([f"{self.response[match]}" for match in matches])
    ...
    def all_matches(self,filtered, matches):
        matching_services = []

        for match in matches:
            if any(f" {word} " in f" {match} " for word in filtered):
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if user_input.lower() in bot.negative_response:
            print("Bot: Goodbye!")
            break
